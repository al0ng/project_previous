...
def loadDataSet():
    ...
    counter2 = 0
    all = []
    rootdir = "E:\Curriculum\DistributedCompute\outPut\IF-IDF_testt"
...
def createC1(dataSet):
    ...
    '''
    C1 = []
    for transaction in dataSet:         #dataSet是一个二维数组,每一行一个transaction代表一个文章
        for item in transaction:        #每个item代表一个词
            if [item] not in C1:
                C1.append([item])       #如果词不在平凡集中则加入
    C1.sort()                           #频繁集排序
...
def scanD(D, Ck, minSupport):
    ...
    for tid in D:                   # 对于每一条transaction
        for can in Ck:              # 对于每一个候选项集can，检查是否是transaction的一部分
...
if __name__ == '__main__':
    ...
    print("\n=======\n")
    # result_file = open('E:\Curriculum\DistributedCompute\Apriori_result1.txt', 'a', encoding='utf-8')
    #
    # for rule in rules:
    #     # print(rule)
    #     # result = str(rule)
    #     result_file.write(str(rule)+'\n')
    #
    # result_file.close()